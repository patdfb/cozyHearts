...
const getInitialProfileFromStorage = () => {
  ...
  if (authData?.usuario) return authData.usuario
  try {
    ...
    <section id="center" className="flex min-h-dvh w-full flex-col pb-10">
      ...
      <div className="px-4">
        ...
        <div className="mt-6 grid gap-3">
          <button type="button" onClick={() => navigate('/main')} className="h-12 rounded-4xl bg-button text-lg font-bold text-white hover:bg-button-selected">
            Voltar ao menu
          </button>
          <button type="button" onClick={handleLogout} className="h-12 rounded-4xl bg-error text-lg font-bold text-white hover:opacity-95">
            Terminar sessao
          </button>
        </div>
      </div>
